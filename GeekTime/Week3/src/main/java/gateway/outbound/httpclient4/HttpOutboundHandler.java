...
public class HttpOutboundHandler {
    ...
    public void handle(final FullHttpRequest fullRequest, final ChannelHandlerContext ctx, List<HttpRequestFilter> filterList) {
        ...
        if (Integer.parseInt(fullRequest.headers().get("code")) > 200) {
            responseError(fullRequest, ctx);
        } else {
    ...
    //过滤器校验并通过 相应错误信息
    private void responseError(FullHttpRequest fullRequest, ChannelHandlerContext ctx) {
        FullHttpResponse response = null;
        try {
            String msg = fullRequest.headers().get("msg");

            byte[] body = msg.getBytes();
            response = new DefaultFullHttpResponse(HTTP_1_1, INTERNAL_SERVER_ERROR, Unpooled.wrappedBuffer(body));
